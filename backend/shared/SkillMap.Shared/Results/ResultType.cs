// <copyright file="ResultType.cs" company="PlaceholderCompany">
...
{
    public static Result<T> FailedToSave<T>(string message) => Result.Failure<T>(ErrorCode.FAILEDTOSAVE, message);

    public static Result<T> FailedToGet<T>(string message) => Result.Failure<T>(ErrorCode.FAILEDTOGET, message);

    public static Result<T> FailedToCreate<T>(string message) => Result.Failure<T>(ErrorCode.FAILEDTOCREATE, message);

    public static Result<T> FailedToDelete<T>(string message) => Result.Failure<T>(ErrorCode.FAILEDTODELETE, message);

    public static Result<T> FailedToUpdate<T>(string message) => Result.Failure<T>(ErrorCode.FAILEDTOUPDATE, message);

    public static Result<T> NotFound<T>(string message) => Result.Failure<T>(ErrorCode.NOTFOUND, message);

    public static Result<T> ValidationError<T>(IEnumerable<string> errors) =>
        Result.Failure<T>(ErrorCode.VALIDATIONERROR, "Validation error: " + string.Join(" | ", errors));
    ...
    public static Result<T> UserWithSuchEmailAlreadyExists<T>(string email) =>
        Result.Failure<T>(ErrorCode.USERALREADYEXISTS, $"User with email {email} already exists");
    ...
    public static Result<T> InvalidPassword<T>(string email) =>
        Result.Failure<T>(ErrorCode.INVALIDPASSWORD, $"Invalid password for user with email {email}");
    ...
    public static Result<T> FailedToSendEmail<T>(string email) =>
        Result.Failure<T>(ErrorCode.EMAILSENDINGFAILED, $"Failed to send email to {email}");

    public static Result<T> InvalidOrExpiredToken<T>() =>
        Result.Failure<T>(ErrorCode.INVALIDOREXPIREDTOKEN, "The provided token is invalid or has expired");
