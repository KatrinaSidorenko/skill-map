...
public static class ResultExtension
{
    ...
    public static bool IsInternalError<T>(this Result<T> result)
        => !result.IsSuccessful && result.Code.Contains(ErrorCode.SYSTEMERRORPREFIX);
    ...
    private static HttpStatusCode ToStatusCode(this string code)
        => code switch
        {
            var c when c.Contains(ErrorCode.USERINPUTERRORPREFIX) => HttpStatusCode.BadRequest,
            var c when c.Contains(ErrorCode.SYSTEMERRORPREFIX) => HttpStatusCode.InternalServerError,
            var c when c.Contains(ErrorCode.NOTFOUND) => HttpStatusCode.NotFound,
            var c when c.Contains(ErrorCode.VALIDATIONERROR) => HttpStatusCode.BadRequest,
    ...
    public static T GetDataOrThrowNotFound<T>(this Result<T> result, string? targetActionMessage = null)
        => result.GetDataOrThrow(ErrorCode.NOTFOUND, targetActionMessage ?? string.Join(result.Code, result.Message, ";"));
