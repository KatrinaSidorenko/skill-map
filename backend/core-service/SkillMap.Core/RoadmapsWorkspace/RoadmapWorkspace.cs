...
public class RoadmapWorkspace : TrackedEntity
{
    ...
    public virtual ICollection<RoadmapLearningProjection> LearningItemProjections { get; set; }
    ...
    public void ActivateAuthorMode() => IsInAuthorMode = true;

    public void SetMetadata(string title, string? description, string? imageUrl)
    {
        Title = title;
    ...
    public string ActualRoadmapId => IsInAuthorMode
        ? PersonalRoadmapId.ToString() ?? throw new ArgumentException("Something went wrong with roadmap id")
        : RoadmapId;
}